...
with open('../outputs/gemini-pro-1.5-0_shot.json', 'w', encoding='utf-8') as f:
    raw_data['results'] = answer_list
    json.dump(raw_data, f, ensure_ascii=False, indent=4)

answer_list = []
for j, data in enumerate(data_list):
    print(f"Processing item {j+i+1}/{len(data_list)}")  # 진행 상태 출력
    try:
        messages = [
...
with open('../outputs/gemini-pro-1.5-1_shot.json', 'w', encoding='utf-8') as f:
    raw_data['results'] = answer_list
    json.dump(raw_data, f, ensure_ascii=False, indent=4)